// The Stage-1 simulated run, kept intact so the deployed demo works with no
// keys, no database, and no spend. Selected by PIPELINE_MODE=mock.
...
/**
 * Drives the three-step progress display on timers, then resolves with a
 * fabricated lead. Returns a cancel function so a leaving component can stop.
 */
export function runSimulatedScoring(
